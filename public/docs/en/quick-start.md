...
The rest needs to be done is open `app/Admin/Contollers/UserController.php`, find `show()`, `grid()` and `form()` method and write few lines of code with `model-grid` and `model-form`,for more detail, please read [model-grid](/en/model-grid.md) and [model-form](/en/model-form.md).
